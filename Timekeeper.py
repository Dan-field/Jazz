...
class Timekeeper:
   def __init__(self, UsrStartTempo=None, UsrBeatsPerBar=None, UsrOSCInput=None): #UsrStartTempo should be an integer, but a float will work too
      ...
      self.label2 = Label("<-- faster       slower -->")
      ...
      # TIMER
      self.t1 = Timer(500, self.metronome, (), False) # this is a recurring timer (note, JEM timer is not the same as Python Timer)
      self.t2 = Timer(500, self.metronome, (), False)
      self.oddBar = True
   ...
   def setManualLatency(self, newValue):
      self.latency_adjustment = newValue
      self.label3.setText("Manual Latency: "+str(newValue))
   ...
   def metronome(self):
      if self.kill is False:
         ...
         #      required_delay = 0
         if self.oddBar is True:
            self.t1 = Timer(required_delay, self.metronome, (), False)
            self.t1.start()
         else:
            self.t2 = Timer(required_delay, self.metronome, (), False)
            self.t2.start()
         if self.count16%(16/self.divisions) == 0: # this is a recognised beat
            self.beat +=1
            if self.beat > self.bpb: # too many beats in this bar
               self.t1.stop() # stop, nothing will happen until another downbeat comes in
               self.t2.stop()
               self.endofBar = True
         self.Player.tap16th()
         self.count16 += 1
